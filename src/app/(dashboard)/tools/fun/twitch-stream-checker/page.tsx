...
import { FiSearch, FiRefreshCw, FiHeart, FiEye, FiClock, FiUser, FiTwitch } from 'react-icons/fi';
    ...
    <Layout>
      <div className="flex flex-col items-center">
        <div className="flex items-center mb-6 text-purple-500">
          ...
          <h1 className="text-3xl font-bold">Twitch Stream Checker</h1>
        </div>
        ...
        {currentStream && (
          <div className="w-full max-w-lg mb-8">
            <div className="flex justify-between items-center mb-4">
              ...
              <button
                onClick={() => currentStream.username && toggleFavorite(currentStream.username)}
                className={`p-2 rounded-full ${
                  isFavorite(currentStream.username || '') 
                    ? 'bg-purple-600 text-white' 
                    : 'bg-gray-700 text-gray-300'
                } hover:bg-purple-500 transition-colors`}
                aria-label={`${isFavorite(currentStream.username || '') ? 'Remove from' : 'Add to'} favorites`}
              >
                <FiHeart />
              </button>
            </div>
            <StreamCard stream={currentStream} onRefresh={() => currentStream.username && checkStream(currentStream.username)} />
          </div>
        )}

        {favorites.length > 0 && (
          <div className="w-full max-w-lg">
            <h2 className="text-xl font-semibold text-white mb-4">Your Favorites</h2>
            <FavoritesList favorites={favorites} onCheck={checkStream} onRemove={toggleFavorite} />
          </div>
        )}
      </div>
    </Layout>
